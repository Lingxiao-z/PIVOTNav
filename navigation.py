...
def run_episode(env: Any, observation: Any, task: dict, phase_root: Path,
    ...
    def schedule(reason: str, step: int) -> str | None:
        ...
        if result.selected_ghost_id is not None:
            previous_selected = selected_id
            candidate_frontiers[result.selected_ghost_id]["state"] = "SELECTED"
            candidate_frontiers[result.selected_ghost_id]["visit_count"] += 1
            if previous_selected != result.selected_ghost_id:
                selected_change_events.append({
                    "step": step,
                    "reason": reason,
                    "old_ghost_id": previous_selected,
                    "new_ghost_id": result.selected_ghost_id,
                    "old_sector": candidate_frontiers[previous_selected]["sector"] if previous_selected in candidate_frontiers else None,
                    "new_sector": candidate_frontiers[result.selected_ghost_id]["sector"],
                })
        return result.selected_ghost_id
