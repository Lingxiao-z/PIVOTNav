...
V3310=Path(os.environ.get(
 ...
 str(ROOT/'output/omniguard_v3310_lightglue_active_reobserve'),
))
...
class LightGlueGeometry:
 def __init__(self,device):
  external_lightglue=V3310/'frozen_inputs/LightGlue'
  public_lightglue=Path(__file__).resolve().parents[2]/'third_party'
  sys.path[:0]=[str(V3310/'scripts'),str(V3310)]
  sys.path.insert(0,str(external_lightglue if external_lightglue.is_dir() else public_lightglue))
