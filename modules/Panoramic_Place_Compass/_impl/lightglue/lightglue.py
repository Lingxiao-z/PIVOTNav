...
if hasattr(torch.amp, "custom_fwd"):
    _custom_fwd = torch.amp.custom_fwd(cast_inputs=torch.float32, device_type="cuda")
else:
...
class LightGlue(nn.Module):
    ...
    def __init__(self, features='superpoint', **conf) -> None:
        ...
        if features is not None:
            ...
            path = Path(str(self.conf.weights))
            if not path.is_file():
                path = Path(__file__).parent / 'weights/{}.pth'.format(self.conf.weights)
