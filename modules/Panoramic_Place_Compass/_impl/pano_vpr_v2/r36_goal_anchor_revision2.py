...
class R36GoalAnchorRevision2PresenceHead(nn.Module):
    ...
    def __init__(
        ...
        # The penalty is conditioned on the frozen set evidence, both independent
        # risk probabilities, and verifier competition statistics.
        self.risk_conditioner = _head(
            feature_dim + 5, self.set_cfg.model_dim // 2, self.set_cfg.dropout
        )
    ...
    def architecture_record(self) -> dict[str, Any]:
        return {
            "schema_version": "r36_goal_anchor_revision2_presence_v1",
    ...
    def forward(
        ...
        risk_input = torch.cat(
            (
                feature,
                boundary_probability.unsqueeze(-1),
                wall_probability.unsqueeze(-1),
                competition,
            ),
            dim=-1,
        )
        risk_penalty = self.revision_cfg.risk_penalty_bound * torch.sigmoid(
            self.risk_conditioner(risk_input).squeeze(-1)
        ) * torch.maximum(boundary_probability, wall_probability)
...
class R36GoalAnchorRevision2Model(nn.Module):
    ...
    def architecture_record(self) -> dict[str, Any]:
        return {
            "schema_version": "r36_goal_anchor_revision2_model_v1",
            "candidate_match_frozen": True,
...
def initialize_r36_goal_anchor_revision2(
    ...
    if payload.get("schema_version") != "r36_goal_anchor_checkpoint_v1":
        raise RuntimeError("revision 2 requires an R36 Goal Anchor revision 1 checkpoint")
    ...
    # Both independent risk heads start from the learned r1 near-risk detector,
    # then diverge under their own labels and losses.
    for target_prefix in ("boundary_risk_head.", "wall_risk_head."):
        for name, value in source.items():
            if name.startswith("near_risk_aux_head."):
                destination = target_prefix + name[len("near_risk_aux_head."):]
    ...
    with torch.no_grad():
        model.set_presence_head.risk_conditioner[-1].weight.zero_()
        model.set_presence_head.risk_conditioner[-1].bias.fill_(-4.0)
    return {
        "schema_version": "r36_goal_anchor_revision2_initialization_v1",
        ...
        "boundary_and_wall_heads_initialized_from_r1_near_risk": True,
        "risk_conditioner_newly_initialized": True,
        "risk_conditioner_initialization": "final weight=0, bias=-4; preserve r1 predictions before training",
