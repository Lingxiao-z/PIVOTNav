...
from modules.Topological_Belief_Cascade.production.goal_policy import NTSGoalNavigator
from modules.Topological_Belief_Cascade.production.event_semantics import (
    GOAL_FINAL_CONFIRMED,
    GHOST_FINAL_CONFIRMED,
...
class NTSArrivalGateAdapter:
    ...
    ``goal`` is the only target that can authorize Stop.  Ghost and historical
    ...
    def begin(self, *, target_kind: str, candidate_frontier_id: str | None = None,
        ...
        if target_kind not in {"goal", "ghost", "existing"}:
        ...
        self.audit.append({"event": "arrival_generation_started", "generation": self.generation,
                           "target_kind": target_kind, "ghost_id": candidate_frontier_id,
                           "existing_node_id": existing_node_id, "event_type": {
                               "goal": GOAL_FINAL_CONFIRMED,
                               "ghost": GHOST_FINAL_CONFIRMED,
    ...
    def apply(self, *, generation: int, final_confirmed: bool, step: int,
        ...
        if not final_confirmed:
            self.audit.append({"event": "arrival_result_rejected", "reason": "not_final_confirmed",
                               ...
                               "node_switched": False, "bpl_updated": False, "stop_authorized": False})
            return False
        if request.target_kind == "ghost":
            node_id = self.policy.on_final_confirmed(step=step, target_kind="ghost", candidate_frontier_id=request.candidate_frontier_id,
