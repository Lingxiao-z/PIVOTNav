"""Online candidate-frontier scheduler for RGB goal navigation.
...
class CandidateFrontierNavigator:
    """Stateful regular/candidate-frontier graph with event-only BPL mutation."""

    schema = "pivotnav_candidate_frontier_topology_v1"
    ...
    def __init__(self, sectors: int = NUM_SECTORS) -> None:
        ...
        self.candidate_frontiers: dict[str, CandidateFrontierNode] = {}
    ...
    def add_root(self, step: int = 0, rgb_path: str | None = None, descriptor_ref: str | None = None) -> int:
        ...
        self.create_candidate_frontiers(node_id, int(step))
        return node_id

    def create_candidate_frontiers(self, parent_id: int, step: int) -> list[str]:
        ...
        for sector in range(NUM_SECTORS):
            ...
            if candidate_frontier_id in self.candidate_frontiers:
                continue
            self.candidate_frontiers[candidate_frontier_id] = CandidateFrontierNode(
                candidate_frontier_id, int(parent_id), sector, int(step)
            )
            created.append(candidate_frontier_id)
            self._event(self.candidate_frontier_events, {"event": "candidate_frontier_created", "candidate_frontier_id": candidate_frontier_id,
    ...
    def update_fs_scores(
        ...
        """Score every non-exhausted candidate frontier and select one target.

        ``scores`` is keyed by candidate-frontier id for online use.  A sequence is accepted
        ...
        candidates: list[CandidateFrontierNode] = []
        evaluated: list[dict] = []
        for candidate_frontier in self.candidate_frontiers.values():
            ...
            evaluated.append({"candidate_frontier_id": candidate_frontier.candidate_frontier_id, "parent_node": candidate_frontier.parent_id,
        ...
        record = {
            ...
            "evaluated_candidate_frontier_count": len(evaluated),
            ...
            "previous_selected_candidate_frontier_id": previous_target,
            ...
            "candidates": [{"candidate_frontier_id": item.candidate_frontier_id, "parent_node": item.parent_id,
            ...
            "all_candidate_frontier_scores": evaluated,
    ...
    def control_target(self) -> dict:
        ...
        A historical candidate frontier is never applied in the current frame: Dijkstra
        first selects the next regular node, and only at its parent is the
        candidate-frontier sector converted to a body-relative bearing.
        """
        ...
        if isinstance(target, str):
            ...
            return {"mode": "FS_CANDIDATE_FRONTIER", "bearing_degrees_right_positive": bearing,
    ...
    def record_branch_feedback(
        ...
        *,
        ...
        branch value and eventually block that candidate frontier until a later global
        replan can reconsider it.
        """
        target = self.long_term_goal
        if not isinstance(target, str) or target not in self.candidate_frontiers:
            return {"updated": False, "reason": "no_active_candidate_frontier"}
        candidate_frontier = self.candidate_frontiers[target]
        ...
        event = {
            "event": "candidate_frontier_branch_feedback",
            "step": int(step),
            "candidate_frontier_id": target,
        ...
        try:
            ...
            if target_kind == "candidate_frontier":
                ...
                if candidate_frontier.state not in {"ACTIVE", "UNKNOWN"}:
                    raise RuntimeError(
                        f"candidate frontier {candidate_frontier_id} is {candidate_frontier.state}"
                    )
                node_id = self.next_node_id
                self.next_node_id += 1
                ...
                self._bpl_update(node_id, int(step), reason,
                                 event_type="CANDIDATE_FRONTIER_FINAL_CONFIRMED", final_confirmed=True)
                self._event(self.node_events, {"event": "candidate_frontier_to_regular_confirmed",
                                               "event_type": "CANDIDATE_FRONTIER_FINAL_CONFIRMED", "candidate_frontier_id": candidate_frontier_id,
    ...
    def payload(self) -> dict:
        return {
            ...
            "candidate_frontier_nodes": [vars(node) for node in self.candidate_frontiers.values()],
            ...
            "candidate_frontier_events": self.candidate_frontier_events,
