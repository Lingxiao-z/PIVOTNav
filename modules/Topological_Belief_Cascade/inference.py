...
class OriginalTopology:
    """Reference global graph update used by the public offline API."""
    ...
    def add_regular_node(self, rgb: np.ndarray, scores: np.ndarray | None = None,
        ...
        for sector in range(12):
            frontier_id = f"n{node_id}s{sector}"
            self.frontiers[frontier_id] = CandidateFrontier(frontier_id, node_id, sector)
    ...
    def select(self, node_id: int) -> str | None:
        ...
        choices = [
            frontier for frontier in self.frontiers.values()
            if frontier.parent_node == node_id
            and frontier.state == "available"
            and node.valid[frontier.sector]
        ]
        if not choices:
            self.selected_frontier = None
            return None
        choices.sort(key=lambda frontier: (-float(node.scores[frontier.sector]), frontier.frontier_id))
        selected = choices[0]
        selected.attempts += 1
    ...
    def select_global(self, scores_by_node: dict[int, dict[str, np.ndarray]]) -> str | None:
        ...
        for node_id, scores in scores_by_node.items():
            node = self.nodes[node_id]
            values = np.asarray(scores["fs_scores"], dtype=np.float32).reshape(12)
            valid = np.asarray(
                scores.get("valid_mask", np.ones(12, dtype=bool)), dtype=bool
            ).reshape(12)
            node.scores, node.valid = values, valid
            for frontier in self.frontiers.values():
                if (frontier.parent_node == node_id and frontier.state == "available"
                        and bool(valid[frontier.sector])):
                    candidates.append((-float(values[frontier.sector]), frontier.frontier_id))
        if not candidates:
            self.selected_frontier = None
            return None
        candidates.sort()
